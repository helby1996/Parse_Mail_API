...
find_messages <- function(path){
  ...
  gm_auth()   
  gm_labels()
  my_m <- gm_messages(search="vladislav.rudin@fsight.io")
...
sql_connect <- function(sql_query){
  connection <- dbConnect(MySQL(), port=3306 ,database = 'fsight' , user = 'fsight', password = 'fsight', host='paz.fsight.io')
...
runjob <- function(){
  resp <- POST(url = 'https://h-vlad-test.fsight.io/griddly/authorize' ,
               add_headers(
  ...
  token<-content(resp)
  toke<-token$id_token
  resp2 <- GET('https://h-vlad-test.fsight.io/griddly/operator/runJob?isExcluded=false&jobScheduleName=Noa_automation&jobType=METER',
